...
function enableRowEditing(row, date, product, size, type) {
    ...
    cells.forEach(cell => {
        if (cell.dataset.field === 'date') {
            const input = document.createElement('input');
            input.type = 'date'; // 날짜 필드를 편집 가능하도록 설정
            input.value = cell.textContent.trim(); // 기존 날짜 값을 설정
            cell.textContent = '';
    ...
    editButton.removeEventListener('click', () => enableRowEditing(row));
    editButton.addEventListener('click', () => saveRowChanges(row));
}
...
function saveRowChanges(row) {
    ...
    cells.forEach(cell => {
        ...
        if (input && field === 'date') {
            updatedData[field] = input.value; // 날짜 필드는 문자열 그대로 저장
            cell.textContent = input.value.trim(); // 셀 업데이트
        }
    });

    // Firebase 경로 설정
    const date = updatedData.date || row.querySelector('[data-field="date"]').textContent.trim(); // 업데이트된 날짜
